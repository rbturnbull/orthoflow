...
def test_warning_ignore_files(run_workflow):
    w = run_workflow("results/intake/input_sources.csv", "--inputs", "alphabet.gb", "--ignore-non-valid-files", expected_dir=warnings_expected_dir)
...
def test_warning_missing_taxa(run_workflow):
    w = run_workflow("logs/warnings/missing_taxa.txt", "--inputs", "input_sources.csv", "--ignore-non-valid-files", expected_dir=warnings_expected_dir)
...
def test_warning_suffix(run_workflow):
    w = run_workflow("results/intake/input_sources.csv", "--ignore-non-valid-files", expected_dir=warnings_expected_dir)
...
def test_warning_configuration_file(run_workflow):
    w = run_workflow("orthofinder -R --inputs input_sources.csv --ortholog-min-seqs=1 --no-supertree --no-supermatrix --ignore-non-valid-files", expected_dir=warnings_expected_dir)
