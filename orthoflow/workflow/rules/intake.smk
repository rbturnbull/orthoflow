...
rule rename_sequences:
    ...
    shell:
        """
        python {SCRIPT_DIR}/rename_sequences.py --debug {input.file} {output} --data-type {params.data_type} --taxon-string  {params.taxon_string} --warnings-dir {WARNINGS_DIR} |& tee {log}
        """
...
rule translate:
    ...
    shell:
        """
        biokit translate_sequence {input} --output {output} --translation_table {params.translation_table} |& tee {log}
        """
